...
description: "记录可复现问题的现象、期望与影响，用于新报告或补充已有报告。"
---
...
2. **看有没有相关 issue 目录**——Glob `.codestable/issues/`，有同类问题按意图复用，仅在目标不清楚时询问
3. **快速通道判断（唯一正式判定点）**——按用户线索**读一下相关代码**（Grep / Read 定位）：
   - **能一眼确定根因**（能给出 `{文件}:{行号}`、修复改动小 1-2 处、无跨模块影响风险）→ 已有修复授权时说明根因和方案并进入 `cs-issue-fix`（快速通道模式）；只要求报告时保持只读
...
- [ ] 复现步骤可执行（"无法稳定复现"也有说明）
- [ ] 期望与范围有用户请求或澄清依据
- [ ] 期望与范围确定时标 `status: confirmed`，否则保留 draft

---

## 退出后

只要求报告时交付报告；已有定位或修复授权则继续 `cs-issue-analyze`，不重复阶段审批。
