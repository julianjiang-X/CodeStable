---
name: cs-trick
description: "将已验证的编程方法整理为处方性参考，说明适用边界与项目证据。"
---
...
**更新流程**：核验受影响证据，按授权范围写回并更新日期；仅在实质取舍未决时询问。
...
证据充分时不扩搜；需要判断适用范围或存在矛盾时定向核验。未验证的做法不可包装成已验证处方。
...
python3 .codestable/tools/search-yaml.py --dir .codestable/compound --filter doc_type=trick --filter type=library --filter framework~={库名}

# 按技术栈浏览

python3 .codestable/tools/search-yaml.py --dir .codestable/compound --filter doc_type=trick --filter language=typescript --filter status=active

# 归档后查重叠

python3 .codestable/tools/search-yaml.py --dir .codestable/compound --filter doc_type=trick --query "{关键词}" --json
```
...
2. **证据充分**——优先使用已验证项目代码；证据不足才定向调查，不重复搜索
