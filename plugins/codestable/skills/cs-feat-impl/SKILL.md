...
> 结构与取舍拿不准时读 `.codestable/reference/economy.md`（最少新增复杂度、有界简化）和 `.codestable/reference/code-design.md`（归属、深度、接缝）。
> 这一步要加多重的验证和审查看 `.codestable/reference/assurance.md`。
...
但**最少 ≠ 最小 diff**：改在最近的位置常常比改在正确的责任边界更贵。动手前按 `economy.md` 的五级梯子走一遍（现有行为能否满足 → 现有模块能否复用 → 标准库 / 平台能力 → 已装依赖 → 才写新代码），命中即停。用了有已知上限的简单方案时，按有界简化三要素（上限 / 触发 / 方向）记进 `{slug}-ff-note.md` 或 design，光写"先这样"不算数。

同时，`economy.md` 也划了不能被"最少"删掉的部分：信任边界校验、防数据丢失的错误处理、危险操作护栏、与失败代价相称的验证证据。
...
- `{slug}-checklist.yaml`、需求来源（用户描述 + brainstorm note）、`.codestable/attention.md`、`.codestable/reference/shared-conventions.md` 第 2.6 节
...
必须使用可用的 subagent reviewer。若当前对话还没有明确 subagent / delegation 授权，先按 shared-conventions 第 2.6 节的 review authorization judgment checkpoint 提供背景、术语、取舍、默认建议和非自动动作；不要等到 review gate 才首次询问。只有平台确实没有 subagent 能力时才允许 fresh self-review fallback，并在 review 文件和汇报里明确说明。把完整结果写入 feature 目录的 `{slug}-implementation-review.md`。reviewer 有 P0 / P1 时先修，再复核。没有这份 review 文件，不输出实现完成汇报。
