...
判断接口该做多深、接缝是否真实、要不要先重切结构，看 `.codestable/reference/code-design.md`。两条最常用的判据：**删除测试**（假想删掉这个模块，复杂度跟着消失就说明它只是穿透层）和**一个 adapter 只是想象中的接缝，两个 adapter 才说明真有变化维度**。
...
- 引入**新词**，或给已有词**重载**新含义；
- 相邻概念的**边界**会改变行为、归属、公开契约或验收。

触发了就把 canonical 术语、定义、**排除含义**和边界场景写进第 1 节"决策与约束"或第 2.1 名词层，或直接链接项目已有的 canonical 定义；同一术语全文同义。

**没触发就跳过**——沿用已有单义词汇的普通改动不新增 glossary、不额外提问、不为术语单独建产物。纯实现细节的命名不属于触发范围。
