...
- 新建：写入 `compound/YYYY-MM-DD-learning-{slug}.md`（日期取**归档当天**），frontmatter 带 `doc_type: learning` + `evidence: observed`
- 更新：写回 Phase 1.5 定位的原文件 + `updated: YYYY-MM-DD`
- supersede：按 `shared-conventions.md` §6 第 5 条处理

**evidence 只能从 `observed` 起步**——本次任务自己踩的坑，不能在同一个任务里标成 `validated`。晋级、退役和应用规则见 `.codestable/reference/evidence-lifecycle.md`。
...
python3 .codestable/tools/search-yaml.py --dir .codestable/compound --filter doc_type=learning --filter track=pitfall --filter severity=high

# 按组件查相关学习点

python3 .codestable/tools/search-yaml.py --dir .codestable/compound --filter doc_type=learning --filter component~={组件名}

# 归档后查重叠

python3 .codestable/tools/search-yaml.py --dir .codestable/compound --filter doc_type=learning --filter tags~={主要 tag} --json
```
...
3. **机械 guard 优先**——这个坑能被一条测试、checker 或 lint 挡住，且当前任务已有实现授权、机械化不扩大 scope，就直接机械化，不再写一条重复的 learning；会扩大 scope 时才写文档 + 建议后续机械化
4. **不自证 validated**——见 `.codestable/reference/evidence-lifecycle.md`
