...
The tool does not merge by itself. It creates a short-lived intent file that
the branch guard recognizes while an owner-authorized main publish is in
progress.
"""
...
def ensure_clean_target(root: Path, target_branch: str, remote: str) -> None:
    ...
    if local_head != remote_head:
        raise RuntimeError(
            f"{target_branch} must match {remote}/{target_branch} before publish: {local_head} != {remote_head}"
        )
