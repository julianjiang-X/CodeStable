...
# Skills live under this prefix in the source repo but install flat, as
# `<installed_root>/<skill>`. source_files() strips the prefix and show_file()
# re-adds it, so every comparison below works in installed-relative paths.
...
def is_source_repo(root: Path) -> bool:
    return (root / source_path("cs-onboard", "SKILL.md")).exists() and (
        root / source_path("using-codestable", "SKILL.md")
    ).exists()
...
def source_files(root: Path, ref: str, skill: str) -> set[str]:
    """Files of `skill` at `ref`, as installed-relative paths (prefix stripped)."""
    result = git_text(root, "ls-tree", "-r", "--name-only", ref, "--", source_path(skill))
    ...
    for line in result.stdout.splitlines():
        entry = line.strip()
        if not entry.startswith(strip):
            continue
        relative = entry[len(strip):]
        if relative and not is_ignored(Path(relative)):
            files.add(relative)
    return files


def show_file(root: Path, ref: str, path: str) -> bytes | None:
    """Read an installed-relative path out of the source tree at `ref`."""
    result = git_bytes(root, "show", f"{ref}:{source_path(path)}")
