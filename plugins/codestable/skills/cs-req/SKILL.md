---
name: cs-req
description: "维护长期需求记录及变更依据，用于能力边界和验收意图的定义或调整。"
---
...
**按情况读**：可能承载这能力的 architecture doc（用于 `implemented_by`）；相关已有 feature 方案；compound 沉淀（`python3 .codestable/tools/search-yaml.py --dir .codestable/compound --query "{能力关键词}"`）。
