...
# Skills live under this prefix in the source repo but are installed flat, as
# `<installed_root>/<skill>`. Every source-side path must go through
# skill_source() / source_relpath(); installed-side paths stay flat.
SKILLS_PREFIX = ("plugins", "codestable", "skills")
...
def skill_source(root: Path, skill: str) -> Path:
    """Source-tree location of a skill inside the plugin package."""
    return root.joinpath(*SKILLS_PREFIX, skill)


def source_relpath(*parts: str) -> str:
    """Repo-relative POSIX path inside the plugin package."""
    return "/".join((*SKILLS_PREFIX, *parts))


def skill_of_path(path: str) -> str | None:
    """Map a repo-relative changed path back to the skill that owns it."""
    parts = Path(path).parts
    prefix_len = len(SKILLS_PREFIX)
    if len(parts) > prefix_len and parts[:prefix_len] == SKILLS_PREFIX:
...
def ensure_source_repo(root: Path) -> list[Finding]:
    required = [
        "README.md",
        source_relpath("cs-onboard", "SKILL.md"),
        source_relpath("codestable-maintainer", "SKILL.md"),
    ]
...
def existing_skill_dirs(root: Path) -> list[str]:
    return sorted(skill for skill in SKILL_DIRS if (skill_source(root, skill) / "SKILL.md").exists())


def is_real_installed_root(path: Path) -> bool:
    """True for roots that serve live agents, which may only be synced from main.

    Covers the `skills` CLI roots and Claude/Codex plugin skill directories. A
    temporary root used for branch verification must not match.
    """
    ...
    if any(resolved == candidate.expanduser().resolve() for candidate in candidates):
        return True
    # Plugin installs land in <plugin dir>/skills under an agent home.
    if resolved.name == "skills":
        for home in (Path.home() / ".claude", Path.home() / ".codex", Path.home() / ".agents"):
            try:
                resolved.relative_to(home.expanduser().resolve())
            except ValueError:
                continue
            return True
    return False
...
PLUGIN_MANIFEST_PREFIXES = (
    "plugins/codestable/.claude-plugin/",
    "plugins/codestable/.codex-plugin/",
)
...
def changed_noninstalled(paths: list[str], skills: list[str]) -> list[dict[str, str]]:
    ...
    result = []
    for path in paths:
        if skill_of_path(path) in skill_roots:
            continue
        if path.startswith(PLUGIN_MANIFEST_PREFIXES):
            # Ships to plugin users even though it is not inside a skill dir.
            result.append(
                {
                    "path": path,
                    "install": "plugin manifest",
                    "reason": "delivered to plugin installs; not synced into flat skill roots",
                }
            )
            continue
...
def validate_skill(clone: Path, skill: str, validator: str | None) -> Finding | None:
    ...
    result = run(["uvx", "--with", "PyYAML", "python", validator, source_relpath(skill)], clone)
...
def run_tests_if_needed(clone: Path, paths: list[str]) -> Finding | None:
    test_prefixes = (
        source_relpath("cs-onboard", "tools") + "/",
        source_relpath("codestable-maintainer", "tools") + "/",
        "tests/",
    )
    needs_tests = any(path.startswith(test_prefixes) for path in paths)
    if not needs_tests:
        return None
...
def run_behavior_suite_if_needed(clone: Path, paths: list[str]) -> Finding | None:
    prefixes = (
        source_relpath("cs") + "/",
        source_relpath("cs-"),
        source_relpath("using-codestable") + "/",
        source_relpath("cs-onboard", "reference") + "/",
        source_relpath("cs-onboard", "tools") + "/",
        source_relpath("codestable-maintainer", "SKILL.md"),
        source_relpath("codestable-maintainer", "references") + "/",
        source_relpath("codestable-maintainer", "scenarios") + "/",
        source_relpath("codestable-maintainer", "tools", "agent-behavior-harness.py"),
    )
    needs_behavior = any(path.startswith(prefixes) for path in paths)
    if not needs_behavior:
        return None
    runner = skill_source(clone, "codestable-maintainer") / "tools/agent-behavior-harness.py"
...
def verify(
    ...
    if clone_result.returncode != 0:
        ...
        for skill in skills:
            source = skill_source(clone, skill)
            dest = installed_root / skill
            if sync_installed:
