...
# This file runs from two layouts and must work in both:
#   source tree     <repo>/plugins/codestable/skills/codestable-maintainer/tools/
#   installed copy  <installed_root>/codestable-maintainer/tools/
# In both, parents[2] is the directory holding sibling skills, so SKILLS_ROOT is
# derived by depth. SOURCE_ROOT (the repo checkout) only exists in the source
# tree; it is found by marker and falls back to SKILLS_ROOT.
# Scenarios use {skills} for the package and {source} for the repo.
SKILLS_ROOT = Path(__file__).resolve().parents[2]


def _find_source_root(skills_root: Path) -> Path:
    """Repo checkout containing skills_root, or skills_root when installed flat."""
    for candidate in skills_root.parents:
        if (candidate / "README.md").is_file() and (candidate / "plugins").is_dir():
            return candidate
    return skills_root


SOURCE_ROOT = _find_source_root(SKILLS_ROOT)
SCENARIO_ROOT = SKILLS_ROOT / "codestable-maintainer/scenarios"
RUNTIME_TOOL_SOURCE = SKILLS_ROOT / "cs-onboard/tools"
RUNTIME_REFERENCE_SOURCE = SKILLS_ROOT / "cs-onboard/reference"
...
def scenario_paths(args: argparse.Namespace) -> list[Path]:
    ...
    for value in args.scenario or []:
        ...
        if not path.is_absolute():
            # Accept both repo-relative ("plugins/codestable/skills/...") and
            # package-relative ("codestable-maintainer/scenarios/...") forms.
            candidates = [SOURCE_ROOT / path, SKILLS_ROOT / path]
    ...
    if not paths:
        # Never return an empty set: a run over zero scenarios would report
        # ok:true with exit 0, which reads as "verified" while checking nothing.
        raise SystemExit(
            f"No scenarios resolved. SCENARIO_ROOT={SCENARIO_ROOT} "
            f"(exists={SCENARIO_ROOT.exists()}), SKILLS_ROOT={SKILLS_ROOT}, "
            f"SOURCE_ROOT={SOURCE_ROOT}. Check the package layout."
        )
    return paths
...
def substitute(value: object, root: Path, work: Path) -> object:
    if isinstance(value, str):
        return (
            value.replace("{skills}", SKILLS_ROOT.as_posix())
            .replace("{source}", SOURCE_ROOT.as_posix())
