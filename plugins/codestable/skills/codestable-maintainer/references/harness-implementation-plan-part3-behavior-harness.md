...
python3 plugins/codestable/skills/codestable-maintainer/tools/agent-behavior-harness.py run \
  --scenario plugins/codestable/skills/codestable-maintainer/scenarios/feat-design-clarify.yaml \
  --runs 3 \
  --actor sterile \
  --json
```
...
Store scenarios under `plugins/codestable/skills/codestable-maintainer/scenarios/`.
...
Extend `plugins/codestable/skills/codestable-maintainer/tools/verify.py` after the runner is stable:

- run behavior regression when changed paths touch `cs-*`, `using-codestable`,
  `cs-onboard/reference/`, `plugins/codestable/skills/cs-onboard/tools/`, or maintainer harness files;
